...
def getListDate(request):  # в соответствии с апи клиента получаем список специалистов через эту функцию
    ...
    list_time_slot = Time_slot.objects.filter(date__gte=data_range.begin, date__lte=data_range.end, user__id=data_range.specialist_id) # в скобках написать конструкцию которая принадлежит указанному диапазону, переданному пользователю, который храниться в data_range
...
def getTimeSlot(request):
    ...
    list_time_slot = Time_slot.objects.filter(date=data_range.date, user__id=data_range.specialist_id)
...
def validationPhoneNumber(request):
    ...
    return Response(validation_phone(validation_phone_request.phone, validation_code)==200)
...
def getApointmentRequest(request):
    ...
    if len(phone_validation) == 0 :
        return Response({"error": "validation code is incorrect"})
    time_slot = Time_slot.objects.filter(id=appointment_request.timeslot_id).first()
